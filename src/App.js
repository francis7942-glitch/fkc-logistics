            ...
            <div style={{padding:'9px 0',borderBottom:'1px solid var(--bd)'}}>
              ...
              {result.dailyRows&&result.dailyRows.length>0&&<div style={{background:'var(--sur2)',borderRadius:'var(--r)',overflow:'hidden',marginTop:4,fontSize:12}}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'100px 1fr 90px 90px 110px',background:'rgba(0,212,170,.06)',borderTop:'2px solid rgba(0,212,170,.2)'}}>
                  <div style={{padding:'8px 10px',fontWeight:700,fontSize:12,color:'var(--ac)',gridColumn:'span 4'}}>Storage Total ({result.dailyRows.filter(r=>r.totalKg>0).length} days billed)</div>
                  <div style={{padding:'8px 10px',fontFamily:'JetBrains Mono',fontWeight:700,fontSize:13,color:'var(--ac)'}}>{fmtM(result.storage)}</div>
                </div>
              </div>}
            </div>
...
function QBOSettings(){
  ...
  const refresh=async()=>{
    setStatus('refreshing');setMsg('');
    try{
      const res=await fetch('/.netlify/functions/qbo-refresh-token',{method:'POST'});
      const d=await res.json();
      if(d.success){
        const exp=new Date(d.expires_at);
        setExpiry(exp.toLocaleTimeString('en-PH',{hour:'2-digit',minute:'2-digit'}));
        setStatus('connected');
        setMsg('Token refreshed! Valid until '+exp.toLocaleTimeString('en-PH',{hour:'2-digit',minute:'2-digit'}));
      } else { setStatus('error');setMsg(d.error||'Refresh failed'); }
    }catch(e){setStatus('error');setMsg(e.message);}
  };
  return(
    <div className="sbox">
      <p className="hint">Connect FKC Logistics to QuickBooks Online to push invoices directly from the Billing page.</p>
      <div style={{background:'var(--sur2)',border:'1px solid var(--bd)',borderRadius:'var(--rl)',padding:'20px 24px',marginBottom:16}}>
        <div style={{display:'flex',alignItems:'center',gap:12,flexWrap:'wrap'}}>
          ...
          <div style={{flex:1}}>
            <div style={{fontFamily:'Syne',fontWeight:700,fontSize:15}}>QuickBooks Online</div>
            {status==='connected'&&<div style={{fontSize:12,color:'var(--ac)',marginTop:2}}>✓ Token valid until {expiry}</div>}
            {status==='error'&&<div style={{fontSize:12,color:'var(--rd)',marginTop:2}}>✗ {msg}</div>}
            {status==='refreshing'&&<div style={{fontSize:12,color:'var(--or)',marginTop:2}}>⏳ Refreshing…</div>}
          </div>
          <button onClick={refresh} className="btn bgh" style={{fontSize:13}} disabled={status==='refreshing'}>🔄 Refresh Token</button>
          <a href="/.netlify/functions/qbo-auth" className="btn" style={{background:'#2CA01C',color:'#fff',textDecoration:'none',padding:'9px 18px',borderRadius:'var(--r)',fontWeight:700,fontSize:13}}>🔗 Reconnect</a>
        </div>
        {status==='connected'&&msg&&<div style={{marginTop:10,fontSize:13,color:'var(--ac)'}}>✓ {msg}</div>}
      </div>
